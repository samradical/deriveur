...
export default class BaseLayer {
  constructor(data) {
    ...
    this._sound.loadedSignal.add(soundId => {
      ...
      Emitter.emit(`ext:${this._id}:soundloaded`)
    })
  ...
  _buildPlaylist(locations, shuffle = true) {
    ...
    this._closestLocation = locations[0] || this._closestLocation
    ...
    let _failedTries = 0
    while (
      this._playlist.length < PLAYLIST_LENGTH &&
  ...
  resume() {
    ...
    this.mapEntering(this.closestLocation)
  }
  ...
  _loadNext(soundOptions, sources, force = false) {
    ...
    if (_source) {
      ...
      Emitter.emit(
        "log:error",
        `_loadNext fail ${this._id} no source`
      )
    }
  }
  ...
  _makeSrcArrayFromLocationUrls(locationUrls) {
    ...
    return locationUrls.slice(0).map(srcArray => {
      return { src: srcArray }
    })
  }
      ...
      sourceData.map(group => {
        return group.map(file => {
          ...
          if (CONFIG.filterOnlyAudioFormats) {
            if (
              file.url.includes(`.${CONFIG.filterOnlyAudioFormats}`)
            ) {
